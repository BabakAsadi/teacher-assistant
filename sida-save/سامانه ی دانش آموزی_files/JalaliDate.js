...
JalaliDate.parse = function (datestring) {
    try {
        if (datestring.jalalidate)
            return new JalaliDate(datestring.jalalidate[0], datestring.jalalidate[1], datestring.jalalidate[2]);
        ...
        if (datestring.indexOf("Date(") > -1) {
            ...
            if (y > 1500) {
                var gd = new JalaliDate().gregorianToJalali(y, m, d);
                return new JalaliDate(gd[0], gd[1] - 1, gd[2]);
            }
            return new JalaliDate(y, m - 1, d);
        }
